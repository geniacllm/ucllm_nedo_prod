...
def dedup_minhashlsh(input_dir: str, output_base: str):
    ...
    for input_file in os.listdir(input_dir):
        ...
        remained_lines.append(
            exec_hojichar_deduplication(
                json_lines, output_base=output_base_for_input, stats=stats
            )
        )

    with open(
        os.path.join(output_base, "results.dedup.jsonl"), "w", encoding="utf8"
    ) as writer:
    ...
    with open(
        os.path.join(output_base, "stats.dedup.jsonl"), "w", encoding="utf8"
    ) as writer:
        for stat in stats:
