...
def __download_config(language: str, index_from: int, index_to: int) -> dict[str, str]:
    ...
    output_file = "{language}_part_{index_from}-{index_to}.jsonl".format(
        language=language,
        index_from=str(index_from).zfill(5),
        index_to=str(index_to).zfill(5),
    )
    return {"filebase": filebase, "output_file": output_file}


def __execute_download(
    language: str, download_file: str, output_file_path: str, dataset_root: str
) -> None:
    ...
    subprocess.run(
        ["git", "lfs", "pull", "--include", f"{language}/{download_file}"], check=True
    )
...
def download_dataset(
    ...
    config = __download_config(
        language=language, index_from=index_from, index_to=index_to
    )
