...
def __execute_download(
    download_file: str, output_file_path: str, dataset_root: str
) -> None:
    ...
    subprocess.run(
        ["git", "lfs", "pull", "--include", f"multilingual/{download_file}"], check=True
    )
...
def download_dataset(
    split: str, output_base: str = "output", index_from: int = 0, index_to: int = 0
) -> None:
